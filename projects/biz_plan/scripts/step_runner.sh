...
DATE=$(date +%Y-%m-%d)
TIME=$(date +%H%M%S)
RUNS_DIR="${RUNTIME_DIR}/runs/${DATE}/challenger"
LOGS_DIR="${RUNTIME_DIR}/logs/${DATE}"
# SUITES_DIR은 settings.sh에서 SAMPLES_DIR로 설정됨
...
run_writer() {
    local sample_file="${SAMPLES_DIR}/${SECTION}_case01.md"
    ...
    # 변수 치환하여 최종 프롬프트 생성 (config/variables.yaml 사용)
    local writer_prompt
    writer_prompt=$(SECTION_ID="$SECTION" PROJECT_DIR="$PROJECT_DIR" PROMPT_FILE="$prompt_file" python3 << 'PYTHON_EOF'
import yaml
import re
import os
...
PYTHON_EOF
    ...
    echo "프롬프트 렌더링 완료 (${#writer_prompt}자)"
...
run_prompt() {
    ...
    # 변수 치환된 기본 템플릿 로드 (config/variables.yaml 사용)
    local base_template section_name topic pages body
    base_template=$(SECTION_ID="$SECTION" PROJECT_DIR="$PROJECT_DIR" python3 << 'PYTHON_EOF'
import yaml
import re
import os
...
생성된 프롬프트:"
        ...
        local prev_eval2_file="${RUNS_DIR}/${SECTION}_v${prev_version}.eval_stage2.json"
        ...
        # Stage2 평가 로드 (있으면)
        local previous_eval2_json=""
        local eval2_block=""
        if [[ -f "$prev_eval2_file" ]]; then
            previous_eval2_json=$(cat "$prev_eval2_file")
            local eval2_score eval2_tags eval2_readiness eval2_improvements
            eval2_score=$(json_get "$previous_eval2_json" "normalized_score")
            eval2_tags=$(json_get_array "$previous_eval2_json" "defect_tags")
            eval2_readiness=$(json_get "$previous_eval2_json" "investment_readiness")
            eval2_improvements=$(echo "$previous_eval2_json" | python3 -c "
import json,sys
d=json.load(sys.stdin)
imps = d.get('priority_improvements', [])
...
[4. Stage2 심층 평가 결과]
═══════════════════════════════════════════════════════════════
- 정규화 점수: ${eval2_score}점
- 결함 태그: ${eval2_tags}
- 투자 검토 가능성: ${eval2_readiness}
- 우선 개선사항:
${eval2_improvements}"
            echo "Stage2 평가 로드: $prev_eval2_file"
        fi
...
" 2>/dev/null)
        ...
        tab6_prompt="당신은 서비스 기획 프롬프트 개선 전문가입니다.
...
[요청사항] ★ 중요 ★
═══════════════════════════════════════════════════════════════
1. 위 평가 결과(Stage1 + Stage2)의 문제점을 해결할 수 있도록 프롬프트를 개선하세요
...
6. 플레이스홀더({topic}, {section_name} 등) 사용 금지 - 위 실제 값을 직접 기입
7. ★★★ 참고문헌 규칙 보존 필수 ★★★
   - 원본 프롬프트의 [사용 가능 근거] 섹션과 URL 정보를 반드시 그대로 유지
   - 참고문헌 형식은 \"[R#] 기관명 (연도). 문서명. URL. (열람일)\" 형태 유지
   - \"사용자 제공 근거\" 또는 \"출처 미제공\" 형식으로 변경 금지
   - 원본에 제공된 URL/기관명/문서명을 환각으로 간주하지 말 것
...
개선된 프롬프트:"
    ...
    # 프롬프트 파일 저장 (GPT 응답 그대로 사용)
    echo "$generated_prompt" > "$prompt_file"
    echo "프롬프트 저장: $prompt_file (${chars}자)"
