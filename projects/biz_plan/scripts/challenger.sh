#!/bin/bash
# challenger.sh - Challenger (개선 프롬프트) 테스트 실행
# 사용법: ./challenger.sh [옵션]
...
show_help() {
    echo ""
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║           Challenger 테스트 실행                             ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo ""
    echo "사용법: ./challenger.sh [옵션]"
    echo ""
    echo "━━━ 옵션 ━━━"
    echo ""
    ...
    echo "━━━ 예시 ━━━"
    echo ""
    echo "  ./challenger.sh                    # 기본 suite-5 실행"
    echo "  ./challenger.sh --suite=suite-s1_2 # 특정 스위트"
    echo "  ./challenger.sh --from=s1_2        # s1_2부터 끝까지"
    echo "  ./challenger.sh --dry-run          # 테스트 모드"
    echo ""
}
...
run_challenger() {
    ...
    echo ""
    echo "━━━ Challenger (개선 프롬프트) 실행 ━━━"
    echo ""

    local DATE=$(date +%Y-%m-%d)
    local RUN_DIR="${PROJECT_DIR}/runtime/runs/${DATE}/challenger"
    local LOG_DIR="${PROJECT_DIR}/runtime/logs/${DATE}"
    mkdir -p "$RUN_DIR" "$LOG_DIR"
    local LOG_FILE="${LOG_DIR}/challenger_$(date +%H%M%S).log"

    echo "📝 로그 파일: $LOG_FILE"

    if [[ "$dry_run" == "--dry-run" ]]; then
        "${PROJECT_DIR}/lib/core/suite_runner.sh" --writer=challenger --suite="$suite" $start $runs $research --dry-run 2>&1 | tee -a "$LOG_FILE"
    else
        "${PROJECT_DIR}/lib/core/suite_runner.sh" --writer=challenger --suite="$suite" $start $runs $research 2>&1 | tee -a "$LOG_FILE"
    fi
}
...
run_from_section() {
    ...
    for section_id in $sections; do
        ...
        local suite_file="${DATA_DIR}/suites/${suite_name}.yaml"
