...
# ══════════════════════════════════════════════════════════════
# 경로 설정 (settings.sh에서 정의된 변수 사용)
# ══════════════════════════════════════════════════════════════
RUNS_DIR="${RUNTIME_DIR}/runs/${TARGET_DATE}/challenger"
# SAMPLES_DIR은 settings.sh에서 export됨
EVALUATOR_STAGE2="${PROMPTS_DIR}/evaluator/evaluator_stage2.md"
...
rescore_file() {
    ...
    if [[ $content_len -lt $MIN_CONTENT_LENGTH ]]; then
        echo "  ⚠️ 콘텐츠 너무 짧음 (${content_len}자 < ${MIN_CONTENT_LENGTH}자) - 스킵"
        return 1
    fi
    ...
    echo "  ChatGPT 호출 중 (Tab $CHATGPT_TAB, timeout: ${TIMEOUT_EVALUATOR:-$DEFAULT_TIMEOUT}초)..."
    ...
    if [[ $chars -lt $MIN_JSON_LENGTH ]]; then
        echo "  ⚠️ JSON 추출 실패 (${chars}자 < ${MIN_JSON_LENGTH}자)"
        return 1
    fi
